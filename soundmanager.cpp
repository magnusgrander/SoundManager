...
void SoundManager::listOpenALDevices(void) {
    ALCdevice* device = nullptr;
    const ALCchar* devices = alcGetString(nullptr, ALC_ALL_DEVICES_SPECIFIER);
    const ALCchar* currentDevice = alcGetString(nullptr, ALC_DEFAULT_DEVICE_SPECIFIER);
    int id = 0;

    std::cout << "Available OpenAL Devices:\n";
    while (*devices) {
        std::cout << id << " - " << devices << (devices == currentDevice ? " (default)" : "") << "\n";
...
ALuint SoundManager::loadOGG(std::string filename)
{
    ifstream soundFile;
    ...
    // Attempt to open the OGG file
    if (ov_fopen(filename.c_str(), &oggFile) < 0)
    {
        cerr << "SoundManager::loadOGG() could not load " << filename << " --- does the file exist?" << endl;
        exit(1);
    }
    ...
    // Allocate memory for data based on the total size
    long totalSize = ov_pcm_total(&oggFile, -1) * vorbisInfo->channels * 2; // 2 bytes per sample
    ...
    do
    {
        bytesRead = ov_read(&oggFile, (char*)data + size, totalSize - size, 0, 2, 1, &current_section);
        size += bytesRead;
    } while (bytesRead > 0 && size < totalSize);

    // Create OpenAL buffer and check for errors
    alGenBuffers(1, &buffer);
    alBufferData(buffer, format, (void*)data, size, frequency);
    error = alGetError();
    if (error != AL_NO_ERROR)
    {
        cerr << "SoundManager::loadOGG() encountered AL error " << (int)error << " when buffering " << filename << endl;
        exit(1);
    }
...
ALuint SoundManager::playSound3D(ALuint buffer, glm::vec3 pos, double refDist, double maxDist)
{
...
ALuint SoundManager::loopSound3D(ALuint buffer, glm::vec3 pos, double refDist, double maxDist)
{
    ALuint source = getUnusedSource();
