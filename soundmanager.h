...
public:
    ...
    ALuint loadOGG(std::string filename);   
	...
	ALuint playSound3D(ALuint, glm::vec3 pos, double refDist, double maxDist);

	// loop sound with default settings
	ALuint loopSound(ALuint);

	// loop sound with specified 3D settings
	ALuint loopSound3D(ALuint, glm::vec3 pos, double refDist, double maxDist);
